...
#    Launch and Login DBS    ${URLQA}    ${username}    ${password}
    FOR    ${dataIterator}    IN RANGE    ${OrderTypeCount}
        ...
        ${Discode}=    get from list    ${DiscountTypeList}    ${ListIndexIterator}
        ${Distype}=    get from list    ${DiscountCodeList}    ${ListIndexIterator}
        ${ActualCurrency}=    get from list    ${currencylist}    ${ListIndexIterator}
        IF    '${Flag}' != 'N'
...
#                        close browser
                        Get Values In VIAX    ${OrderID}
                        go back
                        ${Currency}=    Get Substring    ${TotalAmount}    -3
                        ${TaxAmount}=    Get Substring    ${TaxAmount}    0    -3
                        ${TotalAmount}=    get substring    ${TotalAmount}    0    -3
                        ${Tax}=    convert to string    ${Tax}
                        ${TaxAmount}=    convert to string    ${TaxAmount}
                        ${TotalAmount}=    replace string    ${TotalAmount}    ,    ${EMPTY}
                        ${TotalAmount}=    convert to string    ${TotalAmount}
                        ${Amount}=    convert to string    ${Amount}
                        ${Amount}=    replace string    ${Amount}   ${SPACE}   ${EMPTY}
                        ${TotalAmount}=  replace string    ${TotalAmount}    ${SPACE}    ${EMPTY}
                        ${TaxAmount}=    replace string    ${TaxAmount}    ${SPACE}    ${EMPTY}
                        ${Tax}=    replace string    ${Tax}    ${SPACE}    ${EMPTY}
                        Validate the content and update the excel    ${ActualCurrency}    ${Currency}   HappyFlowInputs  Currency   ${RowCounter}
                        Validate the content and update the excel    ${Tax}    ${TaxAmount}   HappyFlowInputs  Tax   ${RowCounter}
                        Validate the content and update the excel    ${Amount}    ${TotalAmount}   HappyFlowInputs  TotalAmount   ${RowCounter}
                        Validate the content and update the excel    ${DiscountCode}    ${DisType}   HappyFlowInputs  DiscountCode   ${RowCounter}
                        Validate the content and update the excel    ${DiscountType}    ${DisCode}   HappyFlowInputs  DiscountType   ${RowCounter}
...
#            sleep    5s
            ...
            IF    '${InvoiceStatus}' == 'Invoiced' or '${InvoiceStatus}' == 'Completed'
                ...
                log     ${json_content}
                create session    order_session    ${URLQA}    verify=True
                ${headers}=    Create Dictionary    Content-Type=application/json    Authorization=Bearer ${Token1}
                ${response}=     post on session    order_session    url=${BASE_URLQA}    data=${json_content}     headers=${headers}
                # Getting the content value
                Log    Status Code: ${response.status_code}
                Log    Response Content: ${response.content}
                #add Token Verification
...
Get Values In VIAX
    ...
    ${TaxAmount}=    seleniumlibrary.get text    ${TaxAmtText}
    ${TotalAmount}=    seleniumlibrary.get text    ${TotalAmtText}
    ${TaxAmount}=    set suite variable    ${TaxAmount}
    ${TotalAmount}=    set suite variable    ${TotalAmount}
    seleniumlibrary.click element    ${wileyorderdetails}
    sleep    3s
    ${DiscountType}=    seleniumlibrary.get text    ${DiscountTypeText}
